...
export class LensNotFoundError extends Error {
  constructor(handle: string) {
    super(
      `No Lens account found for "@${handle}"`,
    );
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (json.errors?.length) {
        throw new Error(
          `Lens API error: ${json.errors[0]?.message ?? "unknown"}`,
        );
      }
...
export async function resolveAccount(
  ...
  if (!localName)
    throw new LensNotFoundError(handle);
  ...
  let data =
    await lensQuery<AccountQueryResult>(
      ACCOUNT_QUERY,
      { localName },
    );
  if (!data.account) {
 ...
 * clock: for an active account that's identical to "the last 90 days", but for a
 * dormant one it surfaces their most recent 90 days of activity instead of an
...
async function tallyAffinity(
  ...
  while (page < MAX_PAGES) {
    const data: PostsQueryResult =
      await lensQuery<PostsQueryResult>(
        POSTS_QUERY,
        {
          author: address,
          cursor,
    ...
    for (const post of recent) {
      ...
      bump(
        post.commentOn?.author?.username
          ?.localName,
      );
      bump(
        post.quoteOf?.author?.username
          ?.localName,
      );
      for (const h of extractHandles(
...
> {
  ...
  for (
    ...
    chunks.push(
      handles.slice(i, i + ALIAS_CHUNK),
    );
  }

  // One round-trip per chunk of 10, all in flight at once.
  const results = await Promise.all(
    chunks.map(resolveFriendChunk),
  );
...
export async function computeCircle(
  rawHandle: string,
): Promise<CircleData> {
  const handle =
    sanitizeHandle(rawHandle);
  console.log(
    `🚀 Computing Circle of Affinity for @${handle}`,
  );

  const profile =
    await resolveAccount(handle);

  const { counts, postsAnalyzed } =
    await tallyAffinity(
      profile.address,
      profile.handle,
    );
  ...
  const totalMentions = sorted.reduce(
    (sum, m) => sum + m.count,
    0,
  );
  ...
  const accounts =
    await resolveFriendAccounts(
      top.map((f) => f.handle),
    );

  const friends: Friend[] = top.map(
    (f) => {
